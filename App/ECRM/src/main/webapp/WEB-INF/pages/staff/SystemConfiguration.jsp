...
<div class="layout-background">
                                ...
                                <div class="content-tab">
                                    <c:forEach begin="5" end="${listConfig.size() - 1}" var="i">
                                        <div class="group-control" style="position:relative;">
                                            ...
                                            <div class="value" id="value-${listConfig.get(i).key}">${listConfig.get(i).value}</div>
                                            <div style="float: right;">
                                                <div class="btn" style="margin: 0; line-height: 19px; height: 19px;" onclick="$('#content-${listConfig.get(i).key}').css('display','block')"><i class="fa fa-pencil"></i></div>
                                            </div>
                                            <div class="update-div" id="content-${listConfig.get(i).key}"
                                                ...
                                                <input class="number-text" type="text" maxlength="2" id="${listConfig.get(i).key}" style="width: 75px" value="${listConfig.get(i).value}"/>
                                                <div class="btn btn-normal" style="height: 19px; line-height: 19px; margin: 0" onclick="updateConfig('${listConfig.get(i).key}')">Save</div>
                                            </div>
                                        </div>
                                    </c:forEach>
                                </div>
                            ...
                            <div class="content-tab">
                                <c:if test="${listConfig.size() > 0}">
                                    ...
                                    <div class="group-control line">
                                        ...
                                        <div class="value" id="value-${listConfig.get(2).key}">${listConfig.get(2).value}</div>
                                        <div style="float: right;">
                                            <div class="btn" style="margin: 0; line-height: 19px; height: 19px;" onclick="$('#content-${listConfig.get(2).key}').css('display','block')"><i class="fa fa-pencil"></i></div>
                                        </div>
                                        <div class="update-div" id="content-${listConfig.get(2).key}"
                                             style="position:absolute; width: 140px; left: 180px; height: 35px; display: none; z-index: 100; background-color: white">
                                            <input class="number-text" type="text" maxlength="2" id="${listConfig.get(2).key}" style="width: 75px" value="${listConfig.get(2).value}"/>
                                            <div class="btn btn-normal" style="height: 19px; line-height: 19px; margin: 0" onclick="updateConfig('${listConfig.get(2).key}')">Save</div>
                                        </div>
                                    </div>
                                </c:if>
                            </div>
    ...
    <script>
        ...
        function updateConfig(key) {
            var valueK = $("#"+key).val();
            if(valueK == "00") {
                conformData(1, {message: "Mức độ hư hại phải lớn hơn 0!"});
            } else {
