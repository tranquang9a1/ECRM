...
<c:set var="equip" value="${requestScope.DAMAGEDEQUIPMENTS}" />
...
<script>
  ...
  <c:if test="${equip.size() > 0}" >
    <c:forEach begin="0" end="${equip.size()-1}" var="i">
      <c:if test="${(equip[i].position != null || '[0]'.equals(equip[i].position) != false)}">
  ...
  loadRoomEquipment(damageEquip, ${rt.equipment});
  showMap('classroom-map-${room.id}', positionEquipments, ${rt.roomType.verticalRows}, '${rt.roomType.horizontalRows}',
