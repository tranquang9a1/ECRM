...
import com.fu.group10.capstone.apps.staffmobileapp.Utils.NetworkUtils;
...
public class ReportAdapter extends BaseAdapter implements InfinityListView.IInfinityAdapter  {
    ...
    public View getView(int i, View convertView, ViewGroup viewGroup) {
        ...
        if (items.get(i).getUserReport().length() > 10) {
            holder.userReport.setText(items.get(i).getUserReport().substring(0, 9) + "..");
        } else {
        ...
        if (mSelectedItemsIds.get(i)) {
            holder.equipmentImg.setImageResource(R.mipmap.ic_check);
        } else {
            holder.equipmentImg.setImageResource(R.mipmap.ic_speaker);
        }
